...
7. Каждая data model из спецификации (A2) ДОЛЖНА быть определена как класс в ОДНОМ из файлов проекта (НЕ в отдельном data_models.py — такого файла НЕТ если он не указан в архитектуре)
...
ВХОДНЫЕ ДАННЫЕ:
- СИСТЕМНЫЕ СПЕЦИФИКАЦИИ (A2) — бизнес-логика. Соблюдай строго.
- API КОНТРАКТ (A5) — МИНИМУМ. Реализуй ВСЕ функции с указанными сигнатурами.
- ГЛОБАЛЬНЫЙ КОНТЕКСТ — импортируй только те имена, что определены в public API других файлов.

ТРЕБОВАНИЯ:
1. Вспомогательные ПРИВАТНЫЕ функции — разрешены
2. Добавляй полную обработку исключений (try/except с конкретными типами)
3. Избегай бесконечных циклов (всегда предусматривай exit condition)
4. Следуй идиомам {{lang}} (PEP8 для Python, etc.)
5. Используй mock/patch для внешних зависимостей
6. Для нейминга потоков данных, переменных, названий функций, классов ИСПОЛЬЗУЙ ТОЛЬКО АНГЛИЙСКИЙ ЯЗЫК.

ЗАПРЕТ ДУБЛИРОВАНИЯ (КРИТИЧЕСКИ ВАЖНО):
- НИКОГДА не определяй класс или функцию, которая уже существует в ГЛОБАЛЬНОМ КОНТЕКСТЕ другого файла
- Если класс/функция уже определены в другом файле — ИМПОРТИРУЙ через from file import Name
- Это касается data models, менеджеров, утилит — всего что есть в public API других файлов
- Нарушение этого правила приводит к циклическим импортам и конфликтам

ПРАВИЛА ИМПОРТОВ (КРИТИЧЕСКИ ВАЖНО):
- ЗАПРЕЩЕНЫ relative imports (from .module import ...). Используй ТОЛЬКО абсолютные: from module import Name
- Импортируй ТОЛЬКО из файлов проекта, stdlib или пакетов из requirements.txt
- НЕ выдумывай несуществующие модули (data_models, external_lib, utils и т.п. — если их НЕТ в списке файлов проекта)
- НЕ пиши бессмысленные присвоения вида: func = func (self-reference)
- Если data model (класс данных) нужен нескольким файлам — определи его В ОДНОМ файле и импортируй из него

ИНКРЕМЕНТАЛЬНЫЕ ИСПРАВЛЕНИЯ (если предоставлен ТЕКУЩИЙ КОД):
- Исправь ТОЛЬКО конкретные проблемы из фидбэка
- НЕ переписывай файл с нуля — сохрани работающую структуру и логику
- Измени минимум кода для решения замечаний
- Если текущий код уже содержит правильные функции — НЕ удаляй и НЕ переименовывай их
...
КРИТЕРИИ REJECT:
- Любая заглушка (pass, ..., NotImplementedError)
