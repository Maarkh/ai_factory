...
- Для каждого класса опиши __init__ И ВСЕ ПУБЛИЧНЫЕ МЕТОДЫ как отдельные записи в контракте.
- Класс только с __init__ без методов — БЕСПОЛЕЗЕН. Другие файлы не смогут его вызвать.
- Пример: класс VideoStreamProcessor должен иметь __init__, get_frame(), process_frame() и т.д.
- Каждый метод — отдельная запись с signature, description, implementation_hints.
...
ГЛАВНОЕ ПРАВИЛО:
- Если предоставлен ТЕКУЩИЙ КОД и ФИДБЭК — исправь ТОЛЬКО указанные проблемы.
  СОХРАНИ ВСЕ существующие функции, классы и импорты. НЕ удаляй код, который работает.
  Верни ПОЛНЫЙ файл (старый код + твои исправления).
- Если ТЕКУЩЕГО КОДА нет — напиши файл с нуля.
...
6. implementation_hints в A5 — ОБЯЗАТЕЛЬНЫЙ алгоритм. Реализуй ТОЧНО по этим шагам. Отклонение от hints = REJECT.
7. ДОСТУПНЫЕ ПАКЕТЫ показаны в контексте. Импортируй ТОЛЬКО из этого списка + stdlib + файлы проекта.
...
- Контекст ошибки
- ИСТОРИЯ РЕВИЗИЙ — какие изменения уже делались и не помогли
- СТАТУС РАЗРАБОТКИ — какие файлы одобрены, какие застряли и почему
...
8. Изучи ИСТОРИЮ РЕВИЗИЙ — НЕ повторяй изменения, которые уже делались
9. Учитывай СТАТУС РАЗРАБОТКИ: если файл застрял на конкретной ошибке — именно её исправь в спецификации
10. Если файл застрял из-за недостающей зависимости (пакет, модуль) — добавь её в описание компонента
